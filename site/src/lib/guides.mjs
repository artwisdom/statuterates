...
export const GUIDES = [
  {
    ...
    "sections": [
      ...
      },
      {
        ...
        "body_html": "<p>Leave federal court and the tidy single rule dissolves. State post-judgment interest tends to fall into a few broad patterns:</p><ul><li><strong>Fixed statutory rates</strong> — a flat number written into the code that changes only when the legislature amends it. Easy to apply, but it can sit far above or below the market for years.</li><li><strong>Market-linked formulas</strong> — a moving benchmark (a Treasury yield, the prime rate, or a state discount rate) plus a fixed margin, often reset annually or quarterly.</li><li><strong>Dual or claim-specific rates</strong> — a different rate depending on the kind of case: contract versus tort, judgments against a government entity, or a special rate for categories such as medical malpractice.</li></ul><p>Some states fix the rate at entry; others let it float over the life of the judgment. The only safe move is to check the specific jurisdiction. Our <a href=\"/states/\">state judgment-interest hub</a> lists each state's recorded rate, cited rule, and known limitations. State calculators remain withheld until their complete rule histories pass validation.</p>"
      },
      {
