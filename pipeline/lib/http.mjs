...
// Source hostnames are code-reviewed, but an otherwise trusted public source can still be
// misconfigured to redirect a refresh runner to an explicit loopback, link-local, or private
// address. Keep those literal address classes outside the fetcher's authority. Documentation and
// reserved ranges are also rejected so a literal address can never become an ambiguous dependency.
...
function assertHttpUrl(url, context) {
  ...
  if (parsed.username || parsed.password) {
    throw new Error(`${context}: URL credentials are not allowed`);
  }
  const hostname = parsed.hostname.toLowerCase().replace(/\.$/, '');
  if (hostname === 'localhost' || hostname.endsWith('.localhost')
      || hostname === 'localhost.localdomain' || hostname.endsWith('.localdomain')
      || hostname.endsWith('.local') || hostname === 'metadata.google.internal') {
    throw new Error(`${context}: unsafe network target ${parsed.hostname}`);
  }
  const literalAddress = hostname.startsWith('[') && hostname.endsWith(']')
    ? hostname.slice(1, -1)
    : hostname;
  const family = isIP(literalAddress);
  if (family && NON_PUBLIC_ADDRESSES.check(literalAddress, family === 4 ? 'ipv4' : 'ipv6')) {
    throw new Error(`${context}: unsafe network target ${parsed.hostname}`);
  }
  return parsed;
}
