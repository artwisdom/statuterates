...
test('official Iowa history contains 302 exact court-table selections through July 2026', () => {
...
test('Iowa court gate rejects changed anchors, stale tables, and future rows', () => {
  assert.throws(
    () => assertIowaCourtTable([{ effective_date: '2025-12-09', index_value: 3.61, value: 5.61 }], { today: '2026-07-19' }),
    /ends before verified baseline/
  );
  assert.throws(
    () => assertIowaCourtTable([{ effective_date: '2026-07-09', index_value: 9.99, value: 11.99 }], { today: '2026-07-19' }),
    /changed verified/
  );
  assert.throws(
    () => assertIowaCourtTable([{ effective_date: '2027-03-09', index_value: 3.56, value: 5.56 }], { today: '2026-07-19' }),
    /future/
  );
});
